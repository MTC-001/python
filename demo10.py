...
class TSP:
    ...
    def result_path(self):
        ...
        res = ""
        for i in range(self.num-1):
            res+=str(self.pop[self.index][i])
            res+="-->"
        res+=str(self.pop[self.index][-1])
        print(a+res)
        print("the total distance is",self.dis[self.index])
        int_array = [int(x) for x in res.split("-->")]
        show(int_array,citys)
...
def draw(citys):
    ...
    x.append(x[0])
    y.append(y[0])
    plt.plot(x,y,'r-',ls='--',c='g',marker='o',ms=8,mfc='r')
    plt.xlabel('x')
    plt.ylabel('y')

#绘制路径图表
def draw_path(line, citys):
    x, y = [], []
    for i in line:
        city = citys[i]
        x.append(city[0])
...
#显示路径图表
def show(int_array,citys):
    plt.figure(figsize=(12,6))
    plt.subplot(121)
    draw(citys)
    plt.subplot(122)
    draw_path(int_array,citys)
    plt.show()
...
if __name__ == '__main__':
    citys = np.array([[16.47, 96.10],[16.47, 94.44], [20.09, 92.54],
                     ...
                     [17.20, 96.29], [16.30, 97.38], [14.05, 98.12], [16.63, 98.18],
                     [21.52, 95.59], [19.41, 97.13], [20.09, 93.55]])
    main(citys)